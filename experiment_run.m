...
% which are chosen and retieved from:
...
% algorithm
isSolved = false(totalTest, 2); % true if the algorithm find the minimum (relative error less than tolerence)
timeResult = zeros(totalTest, 2); % average time per run (averaging from fastest 5 runs)
iterationResult = zeros(totalTest, 2); % total iteration to terminate
funcEvalResult = zeros(totalTest, 2); % total times of function evaluations per run
timeRatio = zeros(totalTest, 2); % for time performance profile
funcEvalRatio = zeros(totalTest, 2); % for function evaluations performance profile
...
for tn = 1 : totalTest
    ...
    isSolved(tn, 1) = k < maxIter && err < minTol;
    %if err >= minTol
    ...
    isSolved(tn, 2) = k < maxIter && err < minTol;
    %if err >= minTol
